...
local fishingUIActive = false
local fishMongerUIActive = false
local ESX = nil
local QBCore = nil
...
CreateThread(function()
    while true do
        ...
        if fishingUIActive and not showingUI then
...
CreateThread(function()
    while true do
        ...
        if distance < 2.0 then
            sleep = 0
            if not fishMongerUIActive then
                fishMongerUIActive = true
            ...
            if fishMongerUIActive then
                fishMongerUIActive = false
                lib.hideTextUI()
            end
        end
